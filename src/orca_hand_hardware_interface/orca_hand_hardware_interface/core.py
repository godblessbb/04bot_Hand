...
class OrcaHand:
    ...
    def __init__(self, model_path: str = None):
        ...
        self.max_current: int = config.get('max_current', 200)
        self.control_mode: str = config.get('control_mode', 'current_position')
        self.type: str = config.get('type', None)
        
        self.calib_current: str = config.get('calib_current', 20)
        self.wrist_calib_current: str = config.get('wrist_calib_current', 30)
    ...
    def set_joint_torque(self, joint_dict):
        motor_ids = []
        currents = []
        for joint in joint_dict.keys():
            motor_id = self.joint_to_motor_map.get(joint)
            if motor_id is None:
                raise ValueError(f"Joint {joint} not found in joint_to_motor_map.")
            motor_ids.append(motor_id)
            currents.append(joint_dict[joint])

        with self._motor_lock:
            self._dxl_client.write_desired_current(motor_ids, currents)
    ...
    def get_joint_current(self, as_list: bool = True) -> Union[dict, list]:
        motor_cur = self.get_motor_current(True)
        joint_cur = {}
        for motor_id in motor_cur.keys():
            joint_cur[self.motor_to_joint_dict.get(motor_id)] = motor_cur[motor_id]
